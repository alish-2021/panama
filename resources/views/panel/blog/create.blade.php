...
@section('content')
    ...
    <section class="create-block">
        <div class="create">
            <form action="{{ route('blog.store') }}" method="POST" class="create__form" enctype="multipart/form-data">
                @csrf
                ...
                <div class="create__wrap">
                    ...
                    <div class="create__inner">
                        <textarea name="preview_desc" class="create__text">{{ old('preview_desc') }}</textarea>
                        @if (session('errors'))
                            <span class="create__invalid">это поле обязательно для заполнения </span>
                        @endif
                    </div>
